...
configPath = str(ROOT_PATH/'grip_select/mobilenet/ssd_mobilenet_v3_large_coco_2020_01_14.pbtxt')
...
while True:
    ...
    #box_data = list(zip(classIds.flatten(), confs.flatten(), bbox, box_sizes))
        ...
        for i in range(len(box_data)):
            box = box_data[i][2]
            box_offset = (box[0] + 1 / 2 * box[2], box[1] + 1 / 2 * box[3])
            if box_sizes[i] < min_size and box_offset[0] > top_left[0] and box_offset[0] < bottom_right[0] and box_offset[1] > top_left[1] and box_offset[1] < bottom_right[1]:
