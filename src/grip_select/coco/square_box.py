...
for i in range(len(box_data)):
    box = box_data[i][2]
    box_offset = (box[0] + 1 / 2 * box[2], box[1] + 1 / 2 * box[3])
    if box_sizes[i] < min_size and top_left[0] < box_offset[0] < bottom_right[0] and top_left[1] < box_offset[1] < \
            bottom_right[1]:
        ...
        squareBox_bot_right_tup = (square_box[0] + square_box[2], square_box[1] + square_box[3])
        ...
        if x_diff == y_diff:
            ...
            squareBox_bot_right[1] = int(square_box[1] + square_box[3] + (x_diff - y_diff) / 2)
        else:
            squareBox_top_left[0] = int(square_box[0] - (y_diff - x_diff) / 2)
            squareBox_bot_right[0] = int(square_box[0] + square_box[2] + (y_diff - x_diff) / 2)

cv2.rectangle(img, tuple(squareBox_top_left), tuple(squareBox_bot_right), color=(255, 255, 0), thickness=2)
