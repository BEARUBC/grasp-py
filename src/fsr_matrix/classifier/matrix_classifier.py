...
from src.definitions import SETTINGS, TORCH_DEVICE
...
class MatrixClassifier(nn.Module, metaclass=ABCMeta):
    def __init__(self):
        ...
        self._output_map = {0: ObjectShape.NOTHING,
    ...
    def forward(self, x: torch.tensor):
        ...
        if x.size() != self.input_shape:
            raise Exception(
                "Input tensor shape " + str(x.size()) + "does not match expected size " + str(self.input_shape)
            )
