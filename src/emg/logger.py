...
class Logger:
    def __init__(self, measurement_name, df, tags=[]):
        self.measure_name = measurement_name  # name of measurement for writing Pandas DataFrame
    ...
    def influx_write(self):
        with InfluxDBClient(url=self.url, token=self.token, org=self.org) as _client:
            ...
            with _client.write_api(write_options=WriteOptions(batch_size=500, flush_interval=10_000,
                ...
                _write_client.write(self.bucket, self.org, record=pd.DataFrame(self.df),
